...
void toneOutputInit() {
    ...
    HAL_TIM_Base_Start_IT(&TONE_TIMER_0);
    HAL_TIM_Base_Start_IT(&TONE_TIMER_1);
    TONE_TIMER_0.Instance->CR1 &= ~TIM_CR1_CEN;   // disable timer
    TONE_TIMER_1.Instance->CR1 &= ~TIM_CR1_CEN;
...
const TIM_HandleTypeDef *toneTimers[] = {&TONE_TIMER_0, &TONE_TIMER_1};
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
    ...
    if (htim->Instance == TONE_TIMER_0.Instance) {
        if (downcounting) {
            /**
    ...
    else if (htim->Instance == TONE_TIMER_1.Instance) {
        if (!downcounting) {
