...
static uint8_t USBD_MIDI_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum) {
    ...
    pmidi = (USBD_MIDI_ItfTypeDef *)(pdev->pClassSpecificInterfaceMIDI);

    USB_Rx_Cnt = pdev->pPCDHandle->OUT_ep[epnum].xfer_count;

    pmidi->pIf_MidiRx((uint8_t *)&USB_Rx_Buffer, USB_Rx_Cnt);

    USBD_LL_PrepareReceive(pdev, MIDI_OUT_EP, (uint8_t *)(USB_Rx_Buffer),
...
uint8_t USBD_MIDI_RegisterInterface(USBD_HandleTypeDef *pdev, USBD_MIDI_ItfTypeDef *fops) {
    ...
    if (fops != NULL) {
        pdev->pClassSpecificInterfaceMIDI = fops;
        ret = USBD_OK;
    }
